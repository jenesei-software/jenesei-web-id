...
const VITE_DEFAULT_SHORTNAME =
  process.env.VITE_DEFAULT_SHORTNAME || 'Default Shortname'
const VITE_DEFAULT_THEME_COLOR =
  process.env.VITE_DEFAULT_THEME_COLOR || '#ffffff'
const VITE_DEFAULT_DESCRIPTION =
  process.env.VITE_DEFAULT_DESCRIPTION || 'Default Description'

let manifest = fs.readFileSync(manifestTemplatePath, 'utf8')

manifest = manifest.replace('{{VITE_DEFAULT_NAME}}', VITE_DEFAULT_NAME)
manifest = manifest.replace(
  '{{VITE_DEFAULT_SHORTNAME}}',
  VITE_DEFAULT_SHORTNAME,
)
manifest = manifest.replace(
  '{{VITE_DEFAULT_THEME_COLOR}}',
  VITE_DEFAULT_THEME_COLOR,
)
manifest = manifest.replace(
  '{{VITE_DEFAULT_BACKGROUND_COLOR}}',
  VITE_DEFAULT_THEME_COLOR,
)
manifest = manifest.replace(
